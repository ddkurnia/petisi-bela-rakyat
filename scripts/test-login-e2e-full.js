...
(async () => {
  ...
  // STEP 1.5: Create users/{uid} doc NOW (BEFORE sign in) — like setup-admin.ts does in production
  log('STEP 1.5: Create users/{uid} doc with role=super_admin (BEFORE sign in, like setup-admin.ts)');
  const preCreateRes = await httpReq('POST', 8080, `/v1/projects/demo-pbr/databases/(default)/documents/users?documentId=${setupUid}`,
    { fields: { uid: { stringValue: setupUid }, email: { stringValue: 'admin@belarakyat.org' }, role: { stringValue: 'super_admin' }, displayName: { stringValue: 'Test Admin' }, status: { stringValue: 'active' } } },
    { Authorization: `Bearer ${setupToken}` });
  log('  preCreate status:', preCreateRes.status, preCreateRes.status === 200 ? '✓ doc created' : preCreateRes.raw);
